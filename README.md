...
1. デプロイURLにアクセスすると `/login` へ遷移します
2. 初めての方は「新規登録」からアカウントを作成してください
3. メールアドレスとパスワードを入力してログインします

### 2. タスクを作成する

1. タスク一覧画面の「+ タスクを追加」をクリック
2. タイトル（必須・100文字以内）を入力
3. 必要に応じて詳細（500文字以内）・期限日を入力
4. 「タスクを作成する」をクリックして保存
...
|------|------|
| **詳細を見る** | タスク行をクリック |
| **完了・未完了の切り替え** | タスク行左端のチェックボックスをクリック |
| **編集する** | 詳細画面の「編集する」をクリック |
| **削除する** | 詳細画面の「削除する」をクリック → 確認ダイアログで「削除する」 |

### 4. タスクを絞り込む

一覧画面上部のフィルターチップで表示するタスクを切り替えられます。
